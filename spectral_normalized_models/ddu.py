...
def gmm_evaluate(
    net, gaussians_model, loader, device, num_classes, storage_device
):
    num_samples = len(loader.dataset)
...
def gmm_fit(embeddings, labels, num_classes):
    ...
    with torch.no_grad():
        ...
        classwise_cov_features = torch.zeros(
            (num_classes, width, width), dtype=torch.float32
        )

        for c in tqdm(range(num_classes)):
            classwise_cov_features[c] = centered_cov_torch(
