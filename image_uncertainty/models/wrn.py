...
class wide_basic(nn.Module):
    def __init__(
        self, in_planes, planes, dropout_rate, stride=1, mc_dropout=False
    ):
    ...
    def forward(self, x):
        if self.mc_dropout:
            out = nn.functional.dropout(
                self.conv1(F.relu(self.bn1(x))), p=self.dropout_rate
            )
        else:
...
class Wide_ResNet(nn.Module):
    def __init__(
        self, depth, widen_factor, dropout_rate, num_classes, mc_dropout=False
    ):
